...
> [!NOTE]
> A tag's defaults depend on whether the letter is bare or followed
> by a number, so `D` and `R` do not share defaults with
> `D1`/`D2`/... and `R1`/`R2`/...: a bare `D` is diverted power in
> watts, while `D1`/`D2`/... are diversion rates in percent with no
> device class. A bare `T` or an indexed `E` (such as `E1`) is not a
> Mk2PVRouter output and only gets the fallback defaults.

## Binary Sensors

Relay state tags (`R1`, `R2`, ...) can also be exposed as binary
sensors: a value of `0` is published as OFF, any other value as ON.
...
Any tag can also be exposed as a text sensor, which publishes the raw
value exactly as received, without numeric parsing or scaling.
